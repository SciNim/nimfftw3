...
const libPath = currentSourcePath.parentDir().parentDir() / "vendor" / "libs"
when defined(windows):
  const Fftw3Lib* = libPath / "libfftw3-3.dll"
elif defined(macosx):
  const Fftw3Lib* = libPath / "libfftw3(|.0).dylib"
else:
  const Fftw3Lib* = libPath / "libfftw3.so(|.3)"
